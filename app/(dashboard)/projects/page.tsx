...
export default async function ProjectsPage({
  ...
  searchParams: { status?: string; month?: string; search?: string }
}) {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-2 text-sm">
          <Link
            href={`?status=${status === 'ACTIVE' ? 'ARCHIVED' : 'ACTIVE'}&month=${month}`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {withTotals.map((p) => (
          <div key={p.id} className="border rounded p-4">
            <div className="flex items-center gap-2">
              <Link href={`/projects/${p.id}`} className="font-medium text-lg">
                {p.name}
              </Link>
            ...
            <div className="text-sm mt-3">
              This month: <span className="font-medium">{msToHhMm(p.monthTotalMs)}</span>
            </div>
          </div>
        ))}
        {withTotals.length === 0 && <div className="text-sm text-neutral-600">No projects</div>}
      </div>
    </div>
  )
}
