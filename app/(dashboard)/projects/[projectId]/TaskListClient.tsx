...
export default function TaskListClient({ projectId, tasks }: { projectId: string; tasks: TaskItem[] }) {
  ...
  async function createTask() {
    ...
    await fetch("/api/tasks", {
      ...
      body: JSON.stringify({ projectId, name: newTaskName, description: newTaskDesc }),
    });
    setNewTaskName("");
    setNewTaskDesc("");
    router.refresh();
  ...
  async function addManual(taskId: string, start: string, end: string) {
    ...
    await fetch("/api/time-entries", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ taskId, startTime: start, endTime: end, manual: true }),
    });
    router.refresh();
  }
    ...
    <div className="space-y-3">
      <div className="border rounded p-3 space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          ...
          <button className="bg-blue-600 text-white rounded px-3 py-2" onClick={createTask}>Create</button>
        </div>
      </div>

      {tasks.map((t) => (
        <TaskRow key={t.id} task={t} onStart={() => start(t.id)} onStop={() => stop(t.id)} onDelete={() => deleteTask(t.id)} onAddManual={(s, e) => addManual(t.id, s, e)} />
      ))}
...
function TaskRow({ task, onStart, onStop, onDelete, onAddManual }: { task: TaskItem; onStart(): void; onStop(): void; onDelete(): void; onAddManual(start: string, end: string): void; }) {
    ...
    <div className="border rounded p-4 flex flex-col gap-3">
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          {task.activeEntry ? (
            <button className="bg-orange-600 text-white rounded px-3 py-1" onClick={onStop}>Pause</button>
          ) : (
            <button className="bg-green-600 text-white rounded px-3 py-1" onClick={onStart}>Play</button>
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
        ...
        <button className="border rounded px-3 py-2" onClick={() => onAddManual(start, end)}>Add manual time</button>
      </div>
    </div>
  );
}
