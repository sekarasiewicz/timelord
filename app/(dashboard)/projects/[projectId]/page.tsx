...
export default async function ProjectDetail({ params, searchParams }: { params: { projectId: string }; searchParams: { month?: string } }) {
  ...
  const project = await db.project.findFirst({ where: { id: params.projectId, userId } });
  if (!project) return <div className="text-sm text-red-600">Not found</div>;

  const tasks = await db.task.findMany({ where: { projectId: project.id }, orderBy: { createdAt: "desc" } });
  const taskWithTotals = await Promise.all(
    tasks.map(async (t) => {
      ...
      const active = await db.timeEntry.findFirst({ where: { taskId: t.id, endTime: null } });
      return {
        id: t.id,
        name: t.name,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <span className="ml-auto text-sm">This month: <span className="font-medium">{msToHhMm(totalMonth)}</span></span>
        <Link href="/projects" className="text-sm border px-3 py-1 rounded">Back</Link>
      </div>
      <TaskListClient
        projectId={project.id}
        tasks={taskWithTotals.map((t) => ({ id: t.id, name: t.name, description: t.description, monthTotalMs: t.monthTotalMs, activeEntry: t.activeEntry }))}
      />
    </div>
  );
}
