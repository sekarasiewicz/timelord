...
export async function PATCH(req: Request, { params }: { params: { projectId: string } }) {
  ...
  const existing = await db.project.findFirst({ where: { id: params.projectId, userId: session.user.id } });
  if (!existing) return Response.json({ error: "Not found" }, { status: 404 });
  const updated = await db.project.update({ where: { id: existing.id }, data: parsed.data });
